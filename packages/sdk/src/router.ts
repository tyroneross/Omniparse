 ...
 *
 * Provides clear input-type dispatching for local file parsing.
 * The router detects the input type and dispatches to the correct parser:
 *
 ...
 *
 * This ensures no accidental cross-routing between different file types.
 */
 ...
 *
 * This is the core routing logic. It checks:
 * 1. Is it a directory path? → directory
 * 2. Is it a file with a known extension? → specific parser
  ...
  switch (inputType) {
    ...
    case 'unsupported':
      throw new Error(
        ...
        `Supported: .xlsx, .xls, .csv, .pptx, .py, .pdf, or directories.`
      );
  }
}
